# Introductory Statistics

**Author:** Humanitarians AI

**Folder:** `books/introductory-statistics-bundle-with-llms/`

---

## Overview

**Audience.** This book is written for the undergraduate taking their first college statistics course, typically in their second year. They are not math majors. They are studying psychology, biology, business, education, nursing, sociology, criminal justice, economics, public health, or any of the fields where the math of inference is a working tool, not the subject. They have had at least one college math course (often college algebra or pre-calc); some have had calculus.

**Scope.** The thirteen chapters cover, in order:

---

## Table of Contents

<!-- TOC sourced from `_toc.md` -->

*Introductory Statistics — A Working Course*
*Nik Bear Brown*

1. [Sampling and Data](chapters/01-sampling-and-data.md)
2. [Descriptive Statistics](chapters/02-descriptive-statistics.md)
3. [Probability Topics](chapters/03-probability-topics.md)
4. [Discrete Random Variables](chapters/04-discrete-random-variables.md)
5. [Continuous Random Variables](chapters/05-continuous-random-variables.md)
6. [The Normal Distribution](chapters/06-the-normal-distribution.md)
7. [The Central Limit Theorem](chapters/07-the-central-limit-theorem.md)
8. [Confidence Intervals](chapters/08-confidence-intervals.md)
9. [Hypothesis Testing with One Sample](chapters/09-hypothesis-testing-with-one-sample.md)
10. [Hypothesis Testing with Two Samples](chapters/10-hypothesis-testing-with-two-samples.md)
11. [The Chi-Square Distribution](chapters/11-the-chi-square-distribution.md)
12. [The F-Distribution and One-Way ANOVA](chapters/12-f-distribution-and-one-way-anova.md)
13. [Linear Regression and Correlation](chapters/13-linear-regression-and-correlation.md)

---

_Generated overview-and-TOC README. Source files (`book.md`, `outline.md`, etc.) remain the working documents._

---

## What This Book Is

<!-- TODO: populate from chapter content -->

---

## Who This Book Is For

reader's roadmap)

This file is a stub. Sections 1–10 and 12–13 are placeholders for a later pass.
Section 11 (A note about AI) is substantive and written.

A good model for the full version: Pearl's "The Mind Over Data" introduction,
Molnar's Interpretable ML introduction. Both are argument-first and tell the
reader exactly what to expect from each chapter.
-->

# Introduction

<!-- [1] COLD OPEN
     A specific named scene with real stakes.
     No "this book will...", no throat-clearing.

---

## How to Read It

<!-- TODO: populate from chapter content -->

---

## Signature Simulations

| Chapter | Topic | Simulation |
|---------|-------|------------|
| 7 | Chapter 7 | LLM Exercise |

---

## About the Author

**Nik Bear Brown** teaches statistics, AI, and data science at Northeastern University. His PhD in Computer Science from UCLA was in computational and systems biology, with a minor field in statistics. The statistical reasoning this book teaches — probability, distributions, hypothesis tests, regression — runs through his research and his applied teaching.

He is the founder of [Humanitarians AI](https://www.humanitarians.ai/) and the author of the *with LLMs* textbook series. He is also the author of *Computational Skepticism for AI*, a method book on statistical reasoning under AI-augmented analysis.

He works in Boston. [nikbearbrown.com](https://www.nikbearbrown.com) · [irreducibly.xyz](https://irreducibly.xyz) · [skepticism.ai](https://www.skepticism.ai)

---

## Copyright

Copyright © 2026 Nik Bear Brown. All rights reserved.

Published by Bear Brown, LLC.

No part of this publication may be reproduced, distributed, or transmitted in
any form or by any means without the prior written permission of the publisher,
except in the case of brief quotations in critical reviews and certain other
noncommercial uses permitted by copyright law.

ISBN: [INSERT ISBN]

First edition: 2026

