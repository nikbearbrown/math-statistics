# Appendix E — The Chapter Writer Prompt
...
*The Chapter Writer, reproduced for copying.*

---

This is the prompt behind Chapter 7 (*Chapter Writing: The Cowork Draft Run*). It reads your `TIKTOC.md` and `book.md`, audits which chapters already exist (and leaves them alone), reads each chapter's pantry notes, and drafts every unwritten chapter in the book's voice using the default eight-section structure — logging each to `logs/log.csv`. It is discipline-agnostic. Copy it into your project and point it at your book directory. The maintained copy lives in Bear Brown & Company's online prompt library [verify URL at time of writing]; if it differs from this appendix, the online copy is newer.

---

**Runs in:** Cowork or Codex — or Claude Code; any assistant with file read/write to your book directory.

**Dependencies:** `TIKTOC.md`, `book.md`, and the per-chapter pantry notes `pantry/research-ch-*.md` (from Appendix D). *Optional:* `chapters-spec.md`.

**Produces:** `chapters/NN-<slug>.md` for every undrafted chapter; logs to `logs/log.csv`. Existing chapters are left untouched.
