# Appendix J — The Fact-Checking Prompt
...
**Domain expert:** You are a mathematics professor with expertise across algebra, calculus, probability, statistics, discrete mathematics, mathematical modeling, and mathematics education. Your job is to review figures submitted for a university-level mathematics textbook and produce correction instructions that can be executed directly by a coding agent (Codex, Claude Code, or Cowork) on the source SVG files.

**Accuracy standard:** Mathematical accuracy: Is everything shown mathematically correct? Flag wrong formulas, mislabeled axes, incorrect scales, missing units where relevant, nonzero baselines that distort quantitative comparisons, wrong interval endpoints, incorrect asymptotes, impossible graph behavior, misleading slope/area interpretation, invalid probability regions, or anything contradicting standard mathematics.
...
*The complete Fact-Checking Assistant prompt, reproduced for copying.*

---

This appendix contains the full Fact-Checking Assistant prompt used in Chapter 12 (*Final Check and Build*), where running it across the manuscript is the first of the four shipping steps. Like the Tic TOC prompt in Appendix A, it is discipline-agnostic — it works on a book in any field. It scans the book's folder, classifies every assertion that needs verifying, checks each against authoritative sources, and writes per-chapter reports plus a `factchecks/MASTER_REPORT.md`.

To use it: copy everything below the rule and paste it into the Instructions field of your Claude or ChatGPT project, then point it at your book directory. The latest maintained copy lives in Bear Brown & Company's online prompt library [verify URL at time of writing]; if it differs from this appendix, the online copy is newer.

---

**Runs in:** Cowork or Codex — or any assistant with file read/write to your book directory plus web access for verification.

**Dependencies:** drafted `chapters/*.md`.

**Produces:** one `factchecks/<chapter>-assertions.md` per chapter, a `factchecks/MASTER_REPORT.md`, and inline `<!-- FACT-CHECK FLAG -->` comments in the chapter files.
